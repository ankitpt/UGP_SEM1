...
ndf = ndf.sort_values(['SiteName','Azimuth'])
ndf['SectorNum']=ndf.groupby('SiteName')['Azimuth'].rank(method='dense')
#ndf['SectorNum']= ndf.groupby('SiteName')['SectorNum'].rank
ndf['Angle']=0

ndf = ndf.drop_duplicates(subset=['SiteName','Azimuth'])
...
for index, row in enumerate(ndf.iterrows()):
 ...
 try:
     ...
     if(index==len(ndf)-1 or ndf['SectorNum'].values[index+1]==1 ):
         temp=abs(row[1][4]-ndf['Azimuth'].values[int(index-row[1][11]+1)])
         if temp<180 :
...
#       ndf['Angle'][index]=abs(row['Azimuth']-ndf['Azimuth'][index-row['SectorNum']+1])
       temp2=abs(row[1][4]-ndf['Azimuth'].values[index+1])
       if temp2<180:
